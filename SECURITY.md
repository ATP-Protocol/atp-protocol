...
- DUAL provides cryptographic identity, integrity, and state guarantees
...
|---|--------|-----------|-----------|-----------|--------|
| T1 | Authority escalation: Agent claims authority not granted | Authority is verified against org role bindings on every request. No implicit authority. Wallet-to-org binding verified against DUAL. | Gateway + SDK | `gateway.ts:155` (`checkAuthority`), `authority.ts:22-84` | Mitigated |
...
| T7 | Evidence tampering: Post-execution modification of evidence records | Evidence is hash-signed by gateway wallet and anchored on DUAL. `evidence.ts:58-61` computes request/response hashes. DUAL anchoring via `anchor.ts:24-66` creates immutable attestation. Any modification invalidates hash chain. | Gateway + DUAL | `evidence.ts:38-73`, `anchor.ts:24-66`, hashes computed at line 58-61, DUAL anchoring creates attestation_ref | Mitigated |
...
| T10 | Stale delegation: Agent operates under revoked authority | Revocation propagates immediately. Authority checks query live state (not cached). DUAL integration refreshes at `dual/authority.ts` with `cache_ttl` ≤ 60s (Spec 14.6). | Gateway + DUAL | `authority.ts:58` (`hasAuthority` query), DUAL integration at `dual/authority.ts` with TTL enforcement | Mitigated |
| T11 | Cross-org impersonation: Agent claims membership in different org | Wallet-to-org binding verified on DUAL. Federation requires explicit org agreements. Binding is queried on every request; no inheritance across orgs. | Gateway + DUAL | `authority.ts:47-55` (wallet binding check), DUAL org verification in `dual/authority.ts` | Mitigated |
| T12 | Gateway impersonation: Malicious endpoint pretends to be ATP gateway | Gateways authenticate via DUAL wallet signatures. Agents verify gateway identity before submitting. Gateway declares conformance level and identity via TLS cert + DUAL wallet. | Gateway (Operator) | `gateway.ts:384-394` (`getMetadata`), TLS certificate verification is operator responsibility (Spec 13.4) | Operator responsibility |
| T13 | Evidence suppression: Gateway fails to record evidence for sensitive actions | Evidence write failures trigger `evidence_status:pending` with mandatory retry. Persistent failures alert the organization. ATP-Attested conformance requires DUAL anchoring; failure halts escalation. | Gateway | `evidence.ts:66-69`, `anchor.ts:24-66`, failed anchors remain "pending" (line 61) for retry via `retryPendingAnchors` | Partially mitigated (requires operator alerting) |
...
- **Authority isolation:** An agent cannot act outside its declared authority (role, org). Authority is bound to cryptographic identity (DUAL wallet).
...
- **Evidence integrity:** All governed executions produce signed, hashable evidence records. DUAL anchoring makes evidence tamper-evident.
...
2. **DUAL is available and honest.** Wallet lookups, org bindings, and attestations depend on DUAL. If DUAL is compromised, authority and evidence integrity are compromised.
...
If the gateway fails to anchor evidence to DUAL, the evidence record is marked `evidence_status:pending`:
...
**Impact:** If DUAL is unreachable and the gateway crashes before retrying, evidence is lost.  
**Mitigation:** Operators must implement background retry logic (`retryPendingAnchors`) and alerting. ATP guarantees evidence *capture*, not guaranteed *delivery to DUAL*. For ATP-Attested conformance, evidence must be successfully anchored; failures should trigger operational alerts.

### No mTLS Between Agent and Gateway

The reference implementation authenticates agents via DUAL wallet signature, not mutual TLS:

```typescript
// (implied in gateway.ts, auth happens via request.wallet signature)
```

**Impact:** An attacker with network access to the gateway endpoint can send arbitrary requests. The gateway will check authority (via wallet lookup on DUAL), but without mTLS, the attacker can impersonate any wallet they know about.  
...
**Mitigation:** For high-assurance evidence, implement cryptographic timestamping (e.g., RFC 3161) or anchor evidence immediately to DUAL (Spec Section 14.5). DUAL anchoring uses blockchain timestamps, which are tamper-evident.
...
### DUAL Cache Staleness

DUAL wallet and org data is cached with a maximum staleness of 60 seconds:
...
**Mitigation:** For time-critical revocations, operators should set `dual.cache_ttl` to a lower value (e.g., 10s) in `gateway.ts:96`. This increases DUAL load; balance according to your revocation velocity.
...
| Level | Evidence | Attestation | DUAL Integration | Use Case |
|-------|----------|-------------|------------------|----------|
...
| **Attested** | Local + DUAL anchored | Blockchain-backed | Yes, evidence anchored | High-compliance environments, regulated actions |
...
- **Attested:** Suitable for high-compliance environments (finance, healthcare, audit-heavy). Evidence is blockchain-backed and verifiable by external parties.
...
- [ ] **Authority:** Verify DUAL integration is enabled and `cache_ttl` is appropriate for your revocation velocity.
...
- [ ] **Monitoring:** Implement alerting for failed evidence anchoring, stale DUAL cache, and revocation events.
...
- **Spec Section 14:** DUAL Network Integration — wallet authentication, org mapping, evidence attestation
...
- **2026-04-12:** Initial release. Threat model mapped to reference gateway implementation v0.1.0. Conformance level: Verified (local evidence, DUAL integration for identity).
